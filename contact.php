...
<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    if (empty($firstname) || empty($firstname) || empty($firstname) || empty($firstname)) {
      header("Location: ../advancedtemplate.php?contact=empty");
      exit();
    }
    else {
      ...
      if (!preg_match("/^[a-zA-Z]*$/", $firstname) || !preg_match("/^[a-zA-Z]*$/", $lastname)) {
        header("Location: ../advancedtemplate.php?contact=char");
        exit();
      }
      else {
        ...
        if (!filter_var($mailFrom, FILTER_VALIDATE_EMAIL)) {
          header("Location: ../advancedtemplate.php?contact=invalidemail");
          exit();
        }
        else {
          ...
          header("Location: ../advancedtemplate.php?contact=mailsent");
    ...
    header("Location: ../advancedtemplate.php?contact=error");
    exit();
  }
